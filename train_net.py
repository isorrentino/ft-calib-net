...
numpy_dataset = np.concatenate((np.concatenate(
    (dataset['ang_vel'], dataset['lin_acc'], dataset['orientation_quat'], dataset['ft_measured']), axis=1),
                                np.concatenate((dataset['ang_vel'], dataset['lin_acc'], -dataset['orientation_quat'],
                                                dataset['ft_measured']), axis=1)),
    axis=0)
numpy_expected = np.concatenate((dataset['ft_expected'], dataset['ft_expected']), axis=0)

train_examples = np.expand_dims(numpy_dataset, axis=2)
train_labels = np.expand_dims(numpy_expected, axis=2)

train_dataset = tf.data.Dataset.from_tensor_slices((train_examples, train_labels))
x_train, x_valid, y_train, y_valid = train_test_split(train_examples, train_labels, test_size=0.33, shuffle=True)

model = keras.Sequential([
    keras.layers.Reshape(target_shape=(6 + 4 + 3 + 3,), input_shape=(6 + 4 + 3 + 3, 1)),
    keras.layers.Dense(units=6 + 4 + 3,
                       activation='relu',
...
model.fit(x=x_train, y=y_train, epochs=50, batch_size=150, validation_data=(x_valid, y_valid))
