...
if __name__ == "__main__":
    ...
    with h5py.File("../datasets/test_dataset_4.mat", "r") as file:
        dataset["ang_vel"] = (
    ...
    plt.plot(np.array(range(len(predict_dataset[:, 0]))) * 0.01,
             ...
             label="Ground-truth")
    plt.plot(np.array(range(len(predict_dataset[:, 0]))) * 0.01,
              filtfilt(b, a, np.linalg.norm(predict_dataset[:, :3], axis=1) / 9.80665 -3.5+0.2338),
            label="NN")
    plt.plot(np.array(range(len(predict_dataset[:, 0]))) * 0.01,
             filtfilt(b, a, np.linalg.norm(prev_predict[:, :3], axis=1) / 9.80665 - 3.5 + 0.2338),
             label="Linear in-situ calib")
    plt.xlabel("time (s)")
    plt.ylabel("norm(f) / g")
    plt.grid()
